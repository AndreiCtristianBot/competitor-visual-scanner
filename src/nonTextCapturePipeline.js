export async function captureAndMergeNonTextElements(page, nonTextElements, allNonTextElements) {
    const vp = page.viewport() || { width: 1920, height: 1080 };
    const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
    const safeFileName = (s) => String(s || '').replace(/[^a-zA-Z0-9-]/g, '_');
    const writeDebugPng = async (label, suffix, buf) => {
        try {
    ...
    const clampClipToViewport = (clip) => {
     ...
     */
    const refreshRectFromPoint = async (nte) => {
        if (!nte?.rect || nte.rect.width <= 3 || nte.rect.height <= 3)
            return null;
        ...
        if (cx < 0 || cy < 0 || cx > vp.width || cy > vp.height)
            return null;
        try {
            const rect = await page.evaluate((x, y, type) => {
                const pickBest = (start) => {
                    if (!start)
                        return null;
                    ...
                    for (let i = 0; i < 8 && el; i++) {
                        ...
                        if (tag === 'div' || tag === 'span') {
                            ...
                            if (r.width >= 6 && r.height >= 6 && r.width <= 600 && r.height <= 400)
                                return el;
                        }
                        el = el.parentElement;
                    }
    ...
    const captureDirectClip = async (nte, pad, tag = 'DIRECT') => {
        ...
        try {
            ...
            console.log(`[NonText-Capture] ${nte.label} → ${tag} clip FAILED: ${err?.message?.substring(0, 80) || 'unknown'}`);
            return null;
        }
    };
    for (const nte of nonTextElements || []) {
        const dedupeKey = `${nte.type}_${nte.label.toLowerCase()}_${nte.href || nte.src || ''}`;
        if (allNonTextElements.some((e) => `${e.type}_${e.label.toLowerCase()}_${e.href || e.src || ''}` === dedupeKey)) {
            continue;
        }
        try {
             ...
             */
            const isInlineOrHamburger = (nte.type === 'inline-svg-icon' || nte.type === 'hamburger-icon') &&
                nte.rect &&
                nte.rect.width > 5 &&
                nte.rect.height > 5;
            if (isInlineOrHamburger) {
                ...
                if (buf)
                    await writeDebugPng(nte.label, 'ELEM', buf);
                else
                    console.log(`[NonText-Capture] ${nte.label} → FORCE DIRECT clip produced no buffer`);
            ...
            if (nte.src) {
                ...
                const resolvedBg = await page.evaluate((href, src, nteType) => {
                    const toHex = (c) => {
                ...
                const stickyInfo = await page.evaluate((href, src, nteType) => {
                    const toHex = (c) => {
                        const m = c.match(/rgba?\(\s*(\d+),\s*(\d+),\s*(\d+)/);
                        if (!m)
                            return null;
                    ...
                    if (!target)
                        return { hasStickyVariant: false, normalBg: null, stickyBg: null, debug: 'no target found' };
                    ...
                    if (!stickyAncestor)
                        return { hasStickyVariant: false, normalBg: null, stickyBg: null, debug: 'no sticky ancestor' };
                    ...
                    if (!hadSticky) {
                        ...
                        const isStillVisible = targetCs.display !== 'none' &&
                        ...
                        if (!hadIn)
                            stickyAncestor.classList.remove('in');
                        if (!hadScrolled)
                            stickyAncestor.classList.remove('scrolled');
                ...
                console.log(`[Sticky-Debug] ${nte.label}: ${stickyInfo.debug || 'no debug'}`);
                ...
                const captureWithBg = async (bg) => {
                    const ready = await page.evaluate((src, filter, isBg, rW, rH, cW, cH, bgHex) => {
                        try {
                ...
                `;
                  ...
                  `;
                    ...
                    }, nte.src, cssFilter, isBgImage, renderW, renderH, containerW, containerH, bg);
                    if (!ready)
                        return null;
                    await sleep(400);
                    await page.evaluate(() => {
                    ...
                    await sleep(200);
                    const clip = { x: 0, y: 0, width: containerW, height: containerH };
                    const buf = (await page.screenshot({ type: 'png', clip }));
                    await page.evaluate(() => {
                ...
                if (stickyInfo?.hasStickyVariant && stickyInfo?.stickyBg) {
                    const stickyBg = stickyInfo.stickyBg;
                    const stickyBuf = await captureWithBg(stickyBg);
                    if (stickyBuf) {
                        nte.stickyScreenshot = stickyBuf;
                        nte.stickyBgColor = stickyBg;
                        nte.normalBgColor = stickyInfo.normalBg;
                ...
                if ((nte.type === 'ui-control' || nte.type === 'inline-svg-icon' || nte.type === 'hamburger-icon') &&
                    nte.rect &&
                    nte.rect.width > 5 &&
                    nte.rect.height > 5) {
                    ...
                    if (buf)
                        ...
                        console.log(`[NonText-Capture] ${nte.label} → no src, clip too small/failed`);
                }
                else {
...
//# sourceMappingURL=nonTextCapturePipeline.js.map