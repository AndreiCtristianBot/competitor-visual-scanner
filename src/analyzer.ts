...
import type { AnalysisResult, AnalyzeOptions, BackgroundEntry, ImageEntry } from './types.js';
...
async function analyzeUrl(url: string, options: AnalyzeOptions = {}): Promise<AnalysisResult> {
    ...
    const runSafe = async <T>(fn: (...args: any[]) => T, ...args: any[]): Promise<T | null> => {
        try {
    ...
    try {
        ...
        let finalSectionBuffer: Buffer | null = null;
        ...
        const backgrounds: BackgroundEntry[] = Object.entries(globalBgCounts)
            .map(([color, score]) => ({
                color,
                surface: score,
        ...
        const images: ImageEntry[] = Object.entries(globalImageCounts)
            .map(([src]) => ({ src, type: 'image' as const, areaPercent: 'N/A' }))
            .slice(0, 15);
