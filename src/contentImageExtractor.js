export async function collectTopContentImages(page, limit = 10) {
    let topImages = [];
    try {
        ...
        await page.evaluate(() => {
            const markHidden = (el) => {
                const h = el;
                if (h.dataset.codexCookieHidden === '1')
                    return;
        ...
        const imgInfos = await page.evaluate((maxItems) => {
            ...
            return imgs
                ...
                if (fullSrc.includes('icon') ||
                    ...
                    return null;
                if (img.closest('[pnl], #booking, #menu, #mobile, .pnl'))
                    return null;
                if (img.closest('.swiper-slide-duplicate'))
                    return null;
        ...
        for (const info of imgInfos) {
            try {
                const rendered = await page.evaluate((imgSrc, naturalWidth, naturalHeight) => {
                    try {
...
//# sourceMappingURL=contentImageExtractor.js.map