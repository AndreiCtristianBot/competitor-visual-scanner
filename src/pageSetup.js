...
export async function dismissOverlays(page, keepCookies) {
...
`;
export async function performCleanup(page, isInitial, keepCookies) {
    if (keepCookies) {
    ...
    await page.evaluate((initial, css) => {
        ...
        selectors.forEach(s => document.querySelectorAll(s).forEach(e => { e.click(); e.remove(); }));
        ...
        consentSelectors.forEach(s => document.querySelectorAll(s).forEach(e => e.remove()));
        document.querySelectorAll('div, section, aside, footer').forEach(el => {
            ...
            if (!nearBottom)
                return;
            const text = ((el.textContent || '') + ' ' + (el.id || '') + ' ' + (el.className || '')).toLowerCase();
        ...
        document.querySelectorAll('div, section, footer, aside, header, nav').forEach(el => {
            ...
            if (['fixed', 'sticky'].includes(style.position)) {
                ...
                if (isBottom || isTop) {
                    ...
                    const hasHeaderBrandOrMenuIcon = isTop &&
        ...
        if (!initial) {
            document.querySelectorAll('header, nav, .sidebar').forEach(el => {
                ...
                const hasBrandOrMenuIcon = !!el.querySelector('svg, .logo svg, [class*="hamburger"], [class*="menu"], [aria-label*="menu" i]');
                if (hasBrandOrMenuIcon)
                    return;
...
export async function forceRenderContent(page) {
    await page.evaluate(async () => {
        const style = document.createElement('style');
        style.id = 'force-pointer-events-global';
        style.innerHTML = `* { pointer-events: auto !important; }`;
        if (!document.getElementById('force-pointer-events-global'))
            document.head.appendChild(style);
        ...
        `;
        if (!document.getElementById('force-reveal-animations'))
        ...
        document.querySelectorAll('*').forEach(node => {
            ...
            if (!el.style)
                return;
            if (skipOpacityForce.has(el.tagName))
                return;
...
export async function detectStrategy(page) {
    try {
        return await page.evaluate(async () => {
            ...
            window.scrollTo(0, 0);
            await new Promise(r => setTimeout(r, 100));
            const hasSnapMarkers = !!(document.querySelector('.swiper-wrapper') || document.querySelector('.fp-section') ||
            ...
            if ((diff < 70 && scrollHeight > vh + 200) || hasSnapMarkers || fullScreenSections > 2)
                return 'VERTICAL_SNAP';
            return 'STANDARD';
        });
    }
    catch {
...
export async function advancePage(page, strategy, loopCount, viewportH, runSafe) {
    if (strategy === 'HORIZONTAL_APP') {
        const readMainTranslateX = async () => {
            return await page.evaluate(() => {
                const parseTranslateX = (transform) => {
    ...
    // STANDARD
    await runSafe((h) => window.scrollBy(0, h), viewportH);
