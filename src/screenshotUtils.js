...
export async function captureFinalSection(page) {
    try {
...
export async function stitchScreenshots(captureData, strategy, debugDir, finalSectionBuffer) {
    if (!fs.existsSync('./debug_screens'))
        fs.mkdirSync('./debug_screens');
    ...
    try {
        if (captureData.length > 0) {
            ...
            if (finalImage)
                await finalImage.write(`${debugDir}/FULL_PAGE_CAPTURE.jpg`);
        }
        if (finalSectionBuffer) {
            const img = await Jimp.read(finalSectionBuffer);
            await img.write(`${debugDir}/FINAL_SECTION_CAPTURE.jpg`);
