...
export function scanViewportChunk(currentLoopIndex: number, strategy?: string): ViewportScanChunk {
    const _dbg: string[] = [];

    const toHex = (val: string | null): string | null => {
        ...
        if (val.startsWith('rgba')) {
            ...
            if (parts && parts.length === 4 && parseFloat(parts[3]) < 0.05) return null;
        }
    ...
    const isElementViewportVisible = (el: Element, minRatio = 0.02, minW = 8, minH = 8): boolean => {
    ...
    const isRectViewportVisible = (
        rect: { x: number; y: number; width: number; height: number },
        minRatio = 0.02, minW = 8, minH = 8
    ): boolean => {
    ...
    const detectMediaOverlapForRect = (
        rect: { x: number; y: number; width: number; height: number }, contextEl: Element | null
    ): boolean => {
        if (!contextEl || rect.width < 4 || rect.height < 4) return false;
        const overlapsEnough = (a: { x: number; y: number; width: number; height: number }, b: DOMRect): boolean => {
        ...
        const isMediaCarrier = (node: Element): boolean => {
            if (['IMG', 'VIDEO', 'PICTURE', 'CANVAS'].includes(node.tagName)) return true;
            const s = window.getComputedStyle(node);
            if (s.backgroundImage && s.backgroundImage !== 'none' && s.backgroundImage.includes('url')) return true;
            return !!node.querySelector('img, picture, video, canvas');
        };
        const containers: Element[] = [];
        if (contextEl.parentElement) containers.push(contextEl.parentElement);
        if (contextEl.parentElement?.parentElement) containers.push(contextEl.parentElement.parentElement);
        ...
        for (const container of containers) {
            for (const sib of Array.from(container.querySelectorAll('*'))) {
                if (sib === contextEl || sib.contains(contextEl) || contextEl.contains(sib)) continue;
                const ss = window.getComputedStyle(sib);
                if (ss.display === 'none' || ss.visibility === 'hidden' || ss.opacity === '0') continue;
                if (!isMediaCarrier(sib)) continue;
                if (overlapsEnough(rect, sib.getBoundingClientRect())) return true;
            }
        }
        return false;
    };

    const resolveRealColorStructural = (el: Element | null): string => {
        let current = el; let hasImage = false;
        while (current) {
            const style = window.getComputedStyle(current);
            if (style.backgroundImage && style.backgroundImage !== 'none' && style.backgroundImage.includes('url')) hasImage = true;
            if (current.parentElement) {
                for (const sib of current.parentElement.children) {
                    ...
                    if (isOverlay || sib.tagName === 'PICTURE' || sib.tagName === 'IMG') {
                        if (['IMG', 'VIDEO', 'PICTURE'].includes(sib.tagName)) hasImage = true;
                        if (ss.backgroundImage?.includes('url')) hasImage = true;
                        if (sib.querySelector('img, picture, video')) hasImage = true;
                    }
                }
            }
    ...
    const resolveSolidBgOnly = (el: Element | null): string | null => {
    ...
    const detectStructuralImageBackground = (el: Element | null): boolean => {
        ...
        while (current && levels < 8) {
            ...
            if (current.parentElement) {
                ...
                const found = Array.from(current.parentElement.children).some(sib => {
                    if (sib === current) return false;
                    const ss = window.getComputedStyle(sib);
                    const positioned = ss.position === 'absolute' || ss.position === 'fixed';
                    const negZ = parseInt(ss.zIndex) < 0;
                    if (!overlaps(currentRect, sib.getBoundingClientRect())) return false;
                    if (['IMG', 'VIDEO', 'PICTURE', 'CANVAS'].includes(sib.tagName)) return positioned || negZ;
                    if ((positioned || negZ) && ss.backgroundImage?.includes('url')) return true;
            ...
            current = current.parentElement; levels++;
        }
        return false;
    };

    const detectOverlappingMediaSiblings = (el: Element | null): boolean => {
        if (!el) return false;
        const targetRect = el.getBoundingClientRect();
        if (targetRect.width < 4 || targetRect.height < 4) return false;
        const overlapsEnough = (a: DOMRect, b: DOMRect): boolean => {
        ...
        if (el.parentElement?.parentElement) containers.push(el.parentElement.parentElement);
        for (const container of containers) {
            for (const sib of Array.from(container.children)) {
                ...
                if (ss.display === 'none' || ss.visibility === 'hidden' || ss.opacity === '0') continue;
                if (!isMediaCarrier(sib)) continue;
                if (overlapsEnough(targetRect, sib.getBoundingClientRect())) return true;
            }
        }
        return false;
    };

    const hasPseudoMediaOverlay = (el: Element | null): boolean => {
        let current: Element | null = el;
        for (let i = 0; i < 4 && current; i++) {
            const bef = window.getComputedStyle(current, '::before');
            const aft = window.getComputedStyle(current, '::after');
            const hasPseudoBg = (cs: CSSStyleDeclaration) =>
                !!(cs.backgroundImage && cs.backgroundImage !== 'none' &&
                    (cs.backgroundImage.includes('url') || cs.backgroundImage.includes('gradient')));
            if (hasPseudoBg(bef) || hasPseudoBg(aft)) return true;
            current = current.parentElement;
        }
        return false;
    };

    const isOverlayLikeText = (el: Element | null): boolean => {
        let current: Element | null = el;
        for (let i = 0; i < 4 && current; i++) {
    ...
    const hasOpaqueModalAncestor = (el: Element | null): boolean => {
        let current: Element | null = el;
        for (let i = 0; i < 8 && current; i++) {
            const s = window.getComputedStyle(current);
            const role = (current.getAttribute('role') || '').toLowerCase();
            const ariaModal = (current.getAttribute('aria-modal') || '').toLowerCase() === 'true';
            const idClass = `${(current as HTMLElement).id || ''} ${(current as HTMLElement).className || ''}`.toLowerCase();
            ...
            if (isDialogLike) {
                ...
                if (bgHex && coversArea) return true;
            }
            current = current.parentElement;
        }
        return false;
    ...
    const isInFullBleedImageSection = (el: Element | null): boolean => {
        if (!el) return false;
        const section = el.closest('section');
        if (!section) return false;
        const sectionRect = section.getBoundingClientRect();
        if (sectionRect.width < 100 || sectionRect.height < 100) return false;
        ...
        for (const media of mediaEls) {
            ...
            if (mediaArea / sectionArea >= 0.4 && mediaRect.width > sectionRect.width * 0.5) return true;
        }
        return false;
    };

    const finalSection = document.querySelector('#final');
    const getVisualBackgroundAtPoint = (x: number, y: number, textNode: Node | null, gridMode: boolean): string => {
        ...
        for (const el of document.elementsFromPoint(x, y)) {
            ...
            if (skipFinal && finalSection && (el === finalSection || finalSection.contains(el))) continue;
            const hex = toHex(s.backgroundColor);
            const hasBg = s.backgroundImage && s.backgroundImage !== 'none' && s.backgroundImage.includes('url');
            if (gridMode) {
                if (['IMG', 'VIDEO', 'CANVAS', 'PICTURE'].includes(el.tagName)) return 'IMAGE_STACKED';
                if (hex) return hex;
                if (hasBg) return 'IMAGE_STACKED';
                continue;
            }
            if (textNode) {
                ...
                if (!isAnc) {
                    if (['IMG', 'VIDEO', 'CANVAS', 'PICTURE'].includes(el.tagName)) {
                        ...
                        if (textParent) {
                            let tp: Element | null = textParent; let shared = false;
                            for (let i = 0; i < 4 && tp; i++) {
                                if (tp.contains(el)) {
                                    const tag = tp.tagName.toLowerCase();
                                    if (tag !== 'main' && tag !== 'body' && tag !== 'html' && tp.id !== 'scroller' && tp.id !== 'main') shared = true;
                                    break;
                                }
                                tp = tp.parentElement;
                            }
                            if (shared) return 'IMAGE_STACKED';
                        }
                        continue;
                    }
                    if (hex || hasBg) return 'OCCLUDED';
                }
                if (isAnc) {
                    if (['IMG', 'VIDEO', 'CANVAS', 'PICTURE'].includes(el.tagName)) return 'IMAGE_STACKED';
                    const bef = window.getComputedStyle(el, '::before');
                    const aft = window.getComputedStyle(el, '::after');
                    const chkImg = (cs: CSSStyleDeclaration) => cs.backgroundImage && cs.backgroundImage !== 'none' && (cs.backgroundImage.includes('url') || cs.backgroundImage.includes('gradient'));
                    const aftHex = toHex(aft.backgroundColor);
                    if (aftHex) return aftHex;
                    if (chkImg(aft)) return 'IMAGE_STACKED';
                    if (hex) return hex;
                    if (chkImg(s)) return 'IMAGE_STACKED';
                    const befHex = toHex(bef.backgroundColor);
                    if (befHex) return befHex;
                    if (chkImg(bef)) return 'IMAGE_STACKED';
                }
            }
        }
        return 'TRANSPARENT';
    ...
    const resolveEffectiveBg = (centerX: number, centerY: number, node: Node, parent: Element): string | null => {
        let bg = getVisualBackgroundAtPoint(centerX, centerY, node, false);
        const allowHorizontalOverlayPromotion =
            isInFullBleedImageSection(parent) || hasPseudoMediaOverlay(parent);
        const inCookieModal = !!parent.closest(
            '#CybotCookiebotDialog, [id*="CybotCookiebotDialog" i], [class*="CybotCookiebot" i], [id*="cookiebot" i], [class*="cookiebot" i]'
        );
        ...
        if (bg === 'TRANSPARENT' || (bg === '#FFFFFF' && canUseWhiteFallback)) {
            ...
            if (hasOpaqueModalAncestor(parent)) {
                ...
                if (structuralModal && structuralModal !== 'IMAGE') return structuralModal;
            }
            if (detectStructuralImageBackground(parent)) return 'IMAGE_STACKED';
            const structural = resolveRealColorStructural(parent);
            if (structural === 'IMAGE') return 'IMAGE_STACKED';
            if (bg === 'TRANSPARENT') return structural;
        }
        return bg;
    };
    ...
    const localBgs: Record<string, number> = {};
    const localImgs: Record<string, number> = {};
    const viewTextNodes: any[] = [];
    ...
    const cookieDialogRect: { x: number; y: number; width: number; height: number } | null = (() => {
    ...
    const isOccludedByCookieDialog = (rect: { x: number; y: number; width: number; height: number }): boolean => {
        if (!cookieDialogRect) return false;
    ...
    galleryProbe.forEach(el => {
        ...
        if (/open gallery/i.test(txt) && txt.length < 30) {
            ...
            galleryFound = true;
            _dbg.push(
                `[GALLERY-PROBE] loop=${currentLoopIndex} text="${txt}" tag=${el.tagName} class="${el.className}"` +
    ...
    const processedTextKeys = new Set<string>();
    const processedArrowKeys = new Set<string>();
    const normalizeTextForKey = (t: string) => t.toLowerCase().replace(/\s+/g, ' ').trim();
    ...
    while (walker.nextNode()) {
        ...
        const isTopBarTarget = !!(barAnchor && (
            (barAnchor as Element).matches('a.book') ||
            (barAnchor as Element).matches('a.info')
        ));
        if (parent.closest('[pnl], #booking, #menu, #mobile')) continue;
        if (parent.closest('#scrollbar, header, nav')) continue;
        if (parent.closest('#bar') && !isTopBarTarget) continue;
        ...
        try {
            ...
            if (rr.width < 1 || rr.height < 1) {
                if (isGalleryDbg) _dbg.push(`[GALLERY-WALK] "${text}" SKIP: rect ${Math.round(rr.width)}x${Math.round(rr.height)}`);
                continue;
            }
            const PAD = 4;
        ...
        if (processedTextKeys.has(posKey)) continue;
        ...
        if (!isInsideCookieDialog && isOccludedByCookieDialog(rect)) {
            if (isGalleryDbg) _dbg.push(`[GALLERY-WALK] "${text}" SKIP: occluded by cookie dialog`);
            continue;
        }
        ...
        if (!visible) {
            if (isGalleryDbg) _dbg.push(`[GALLERY-WALK] "${text}" SKIP: not visible rect=(${Math.round(rect.x)},${Math.round(rect.y)},${Math.round(rect.width)}x${Math.round(rect.height)}) iw=${Math.round(interW)} ih=${Math.round(interH)} ratio=${visibleRatio.toFixed(2)}`);
            continue;
        }

        const cs = getComputedStyle(parent);
        if (cs.visibility === 'hidden' || cs.display === 'none') {
            if (isGalleryDbg) _dbg.push(`[GALLERY-WALK] "${text}" SKIP: vis=${cs.visibility} disp=${cs.display}`);
            continue;
        }
        if (cs.opacity === '0') {
            const inFullBleed = isInFullBleedImageSection(parent);
            if (!inFullBleed) {
                if (isGalleryDbg) _dbg.push(`[GALLERY-WALK] "${text}" SKIP: opacity=0 fullBleed=false`);
                continue;
            }
            (parent as HTMLElement).style.setProperty('opacity', '1', 'important');
            if (isGalleryDbg) _dbg.push(`[GALLERY-WALK] "${text}" RESCUE: opacity=0 but fullBleed=true → forced 1`);
        }

        if (strategy === 'HORIZONTAL_APP' && !isElementViewportVisible(parent, 0.02, 8, 8)) {
            if (isGalleryDbg) _dbg.push(`[GALLERY-WALK] "${text}" SKIP: not viewport-visible (HORIZ check)`);
            continue;
        }
        ...
        if (effectiveBg === null) {
            ...
            if (inBelnordFooter) {
                ...
                if (fallbackBg && fallbackBg !== 'TRANSPARENT') effectiveBg = fallbackBg;
            }
        }
        ...
        if (effectiveBg !== null && effectiveBg !== 'IMAGE_STACKED') {
            if (isInFullBleedImageSection(parent)) {
                if (isGalleryDbg) _dbg.push(`[GALLERY-WALK] "${text}" OVERRIDE bg=${effectiveBg} → IMAGE_STACKED (fullBleed)`);
                effectiveBg = 'IMAGE_STACKED';
            }
        }
        if (effectiveBg === null) {
            if (isInFullBleedImageSection(parent)) {
                if (isGalleryDbg) _dbg.push(`[GALLERY-WALK] "${text}" OVERRIDE bg=null → IMAGE_STACKED (fullBleed)`);
                effectiveBg = 'IMAGE_STACKED';
            }
        }
        ...
        if (strategy === 'HORIZONTAL_APP' && effectiveBg === 'IMAGE_STACKED') {
            ...
            const mediaOverlap =
                detectMediaOverlapForRect(rect, parent) ||
                detectMediaOverlapForRect(rect, parent.parentElement);
            if (!(inFullBleed || (overlayLike && mediaOverlap))) {
                if (isGalleryDbg) _dbg.push(`[GALLERY-WALK] "${text}" SKIP: IMAGE_STACKED without strong overlay evidence`);
                continue;
            }
        }
        ...
        if (isGalleryDbg) {
            _dbg.push(
                `[GALLERY-WALK] "${text}" FOUND loop=${currentLoopIndex} bg=${effectiveBg} color=${toHex(cs.color)}` +
        ...
        if (effectiveBg !== null) {
            ...
            if (isGalleryDbg) _dbg.push(`[GALLERY-WALK] "${text}" DROPPED: bg=null after all checks`);
        }
    }

    // ── HORIZONTAL_APP fallback: overlay labels ──
    if (strategy === 'HORIZONTAL_APP') {
        const clampRectToViewport = (rect: { x: number; y: number; width: number; height: number }) => {
        ...
        const buildEstimatedTextRect = (el: Element, cs: CSSStyleDeclaration, text: string) => {
            ...
            if (align.includes('left') || align.includes('start')) x = visLeft + 8;
            if (align.includes('right') || align.includes('end')) x = visRight - estW - 8;
        ...
        const getTightTextRect = (el: Element): { x: number; y: number; width: number; height: number } | null => {
            ...
            while (tw.nextNode()) {
                ...
                try {
                    ...
                    for (const cr of Array.from(r.getClientRects())) {
                        ...
                        maxX = Math.max(maxX, cr.x + cr.width); maxY = Math.max(maxY, cr.y + cr.height);
                    }
                } catch {}
            }
        ...
        for (const el of overlayCandidates) {
            if (el.closest('[pnl], #booking, #menu, #mobile, #bar, #scrollbar, header, nav')) continue;
            const cs = window.getComputedStyle(el);
            ...
            if (isInvisibleFont || isTextIndentHidden) {
                ...
                if (/gallery/i.test(rawT)) _dbg.push(`[GALLERY-FB] "${rawT}" SKIP: invisible font trick (font=${cs.font?.substring(0,30)} textIndent=${cs.textIndent})`);
                continue;
            }

            const rawText = (el.textContent || '').replace(/\s+/g, ' ').trim();
            if (rawText.length < 3 || rawText.length > 48 || !/[A-Za-z]/.test(rawText)) continue;
            ...
            if (!textColor) {
                ...
                if (inFB) {
                    textColor = '#FFFFFF';
                    if (isGalleryDbg) _dbg.push(`[GALLERY-FB] "${rawText}" textColor=null → fallback #FFFFFF (fullBleed, css.color=${cs.color})`);
                } else {
                    if (isGalleryDbg) _dbg.push(`[GALLERY-FB] "${rawText}" SKIP: textColor=null (color=${cs.color})`);
                    continue;
                }
            }
            ...
            if (!isOverlay) {
                if (isGalleryDbg) _dbg.push(`[GALLERY-FB] "${rawText}" SKIP: not overlay, not fullBleed`);
                continue;
            }
            ...
            if (processedTextKeys.has(textDedupeKey)) continue;
            ...
            if (hasRealTextRect && (rect.width < 8 || rect.height < 8)) {
                if (isGalleryDbg) _dbg.push(`[GALLERY-FB] "${rawText}" SKIP: real text rect is off-screen after clamp`);
                continue;
            }
            ...
            if (hasRealTextRect && rawText.length <= 40 && (rect.width > 520 || rect.height > 180)) {
                if (isGalleryDbg) _dbg.push(`[GALLERY-FB] "${rawText}" SKIP: suspicious real text box ${Math.round(rect.width)}x${Math.round(rect.height)}`);
                continue;
            }
            ...
            if (!isRectViewportVisible(rect, 0.08, 16, 10)) {
                if (isGalleryDbg) _dbg.push(`[GALLERY-FB] "${rawText}" SKIP: not vp-visible rect=(${Math.round(rect.x)},${Math.round(rect.y)},${Math.round(rect.width)}x${Math.round(rect.height)})`);
                continue;
            }
            ...
            if (!el.closest('#CybotCookiebotDialog, [id*="onetrust" i], [id*="usercentrics" i], [id*="cookie" i][role="dialog"], [id*="consent" i][role="dialog"]')) {
                if (isOccludedByCookieDialog(rect)) {
                    if (isGalleryDbg) _dbg.push(`[GALLERY-FB] "${rawText}" SKIP: occluded by cookie dialog`);
                    continue;
                }
            }

            let effectiveBg: string | null;
            if (inFullBleed) {
                effectiveBg = 'IMAGE_STACKED';
            } else {
                const hasMediaOverlap = detectMediaOverlapForRect(rect, el) || detectMediaOverlapForRect(rect, parent);
                if (!hasMediaOverlap) { if (isGalleryDbg) _dbg.push(`[GALLERY-FB] "${rawText}" SKIP: no media overlap`); continue; }
                effectiveBg = resolveEffectiveBg(rect.x + rect.width / 2, rect.y + rect.height / 2, el.firstChild || el, el);
                if (!effectiveBg || effectiveBg !== 'IMAGE_STACKED') { if (isGalleryDbg) _dbg.push(`[GALLERY-FB] "${rawText}" SKIP: bg=${effectiveBg}`); continue; }
            }

            const isInFinal = !!el.closest('#final');
            if (isGalleryDbg) _dbg.push(`[GALLERY-FB] "${rawText}" FOUND loop=${currentLoopIndex} bg=${effectiveBg} color=${textColor} fullBleed=${inFullBleed}`);
    ...
    for (const el of document.querySelectorAll('a, div, span, button, li')) {
        if (el.closest('[pnl], #booking, #menu, #mobile')) continue;
        const style = window.getComputedStyle(el);
        if (style.display === 'none' || style.visibility === 'hidden' || style.opacity === '0') continue;
        const after = window.getComputedStyle(el, '::after');
        if (!after.backgroundImage?.includes('url')) continue;
        const w = parseFloat(after.width), h = parseFloat(after.height);
        if (isNaN(w) || w <= 5 || isNaN(h) || h <= 5) continue;
        ...
        } else { ax = pRect.right - w; ay = pRect.top + (pRect.height - h) / 2; }
        const PAD = 10;
        ...
        if (!isVisible || processedArrowKeys.has(arrowKey)) continue;
        const isInFinal = !!el.closest('#final');
        let bg = getVisualBackgroundAtPoint(fRect.x + fRect.width / 2, fRect.y + fRect.height / 2, null, true);
        if (bg === 'TRANSPARENT' || bg === '#FFFFFF') bg = 'IMAGE_STACKED';
        viewTextNodes.push({
    ...
    const processedNonTextKeys = new Set<string>();
    
    // Helper to normalize color values to #RRGGBB hex format
    const normalizeToHex = (color: string | null): string | null => {
    ...
    const computeFilteredColor = (filterStr: string): string | null => {
        if (!filterStr || filterStr === 'none') return null;
        const fns: { name: string; value: number }[] = [];
        ...
        while ((match = regex.exec(filterStr)) !== null) {
            let val = parseFloat(match[2]); if (match[2].includes('%')) val /= 100;
            if (match[1] === 'hue-rotate') val = parseFloat(match[2]);
        ...
        for (const fn of fns) {
            switch (fn.name) {
                case 'invert': r=r*(1-fn.value)+(255-r)*fn.value; g=g*(1-fn.value)+(255-g)*fn.value; b=b*(1-fn.value)+(255-b)*fn.value; break;
                case 'sepia': { const a=fn.value,sr=.393*r+.769*g+.189*b,sg=.349*r+.686*g+.168*b,sb=.272*r+.534*g+.131*b; r=r*(1-a)+sr*a; g=g*(1-a)+sg*a; b=b*(1-a)+sb*a; break; }
                case 'saturate': { const s=fn.value,gr=.2126*r+.7152*g+.0722*b; r=gr+s*(r-gr); g=gr+s*(g-gr); b=gr+s*(b-gr); break; }
                case 'hue-rotate': { const a=fn.value*Math.PI/180,co=Math.cos(a),si=Math.sin(a); const rr=(.213+co*.787-si*.213)*r+(.715-co*.715-si*.715)*g+(.072-co*.072+si*.928)*b; const gg=(.213-co*.213+si*.143)*r+(.715+co*.285+si*.140)*g+(.072-co*.072-si*.283)*b; const bb=(.213-co*.213-si*.787)*r+(.715-co*.715+si*.715)*g+(.072+co*.928+si*.072)*b; r=rr;g=gg;b=bb; break; }
                case 'brightness': r*=fn.value;g*=fn.value;b*=fn.value; break;
                case 'contrast': { const c=fn.value; r=(r-128)*c+128; g=(g-128)*c+128; b=(b-128)*c+128; break; }
                case 'grayscale': { const a=fn.value,gr=.2126*r+.7152*g+.0722*b; r=r*(1-a)+gr*a; g=g*(1-a)+gr*a; b=b*(1-a)+gr*a; break; }
            }
        }
    ...
    for (const el of document.querySelectorAll('a, button')) {
        ...
        if (!fontZero && !colorTransparent && !textIndentHidden && !overflowHidden) continue;
        const rect = el.getBoundingClientRect();
        if (rect.width < 5 || rect.height < 5 || rect.y + rect.height < 0 || rect.y > vh || rect.x + rect.width < 0 || rect.x > vw) continue;
        const key = `nontext_bgimg_${Math.round(rect.x)}_${Math.round(rect.y)}`;
        if (processedNonTextKeys.has(key)) continue;
        ...
        if (classes.includes('season') || classes.includes('toggle')) elType = 'ui-control';
        nonTextElements.push({
            ...
            captureIndex: currentLoopIndex, href: (el as HTMLAnchorElement).href || null
        });
        processedNonTextKeys.add(key);
    }

    for (const img of document.querySelectorAll('a img, button img') as NodeListOf<HTMLImageElement>) {
        if (img.closest('[pnl], #booking, #menu, #mobile')) continue;
        const rect = img.getBoundingClientRect();
        if (rect.width < 1 || rect.height < 1 || rect.y + rect.height < 0 || rect.y > vh || rect.x + rect.width < 0 || rect.x > vw) continue;
        const key = `nontext_img_${Math.round(rect.x)}_${Math.round(rect.y)}_${img.src.split('/').pop()}`;
        if (processedNonTextKeys.has(key)) continue;
        const cs = window.getComputedStyle(img);
        if (cs.display === 'none' || cs.visibility === 'hidden') continue;
        if (rect.width <= 2 || rect.height <= 2 || (rect.width > 250 && rect.height > 250)) continue;
    ...
    for (const slider of document.querySelectorAll(
        '#CybotCookiebotDialog .CybotCookiebotDialogBodyLevelButtonSlider, [id*="cookiebot" i] .CybotCookiebotDialogBodyLevelButtonSlider'
    )) {
        const cs = window.getComputedStyle(slider);
        if (cs.display === 'none' || cs.visibility === 'hidden' || cs.opacity === '0') continue;
        const rect = slider.getBoundingClientRect();
        if (rect.width < 12 || rect.height < 10) continue;
        if (rect.y + rect.height < 0 || rect.y > vh || rect.x + rect.width < 0 || rect.x > vw) continue;
        ...
        const labelText = (
            wrapper?.querySelector('label strong')?.textContent ||
        ...
        nonTextElements.push({
            ...
            tagName: (slider as Element).tagName,
            src: '',
            cssFilter: cs.filter !== 'none' ? cs.filter : null,
    ...
    for (const svg of document.querySelectorAll('header svg, .logo svg, nav svg, .navbar-brand svg') as NodeListOf<SVGElement>) {
        const rect = svg.getBoundingClientRect();
        if (rect.width < 10 || rect.height < 10 || rect.y + rect.height < 0 || rect.y > vh || rect.x + rect.width < 0 || rect.x > vw) continue;
        const key = `nontext_svg_${Math.round(rect.x)}_${Math.round(rect.y)}`;
        if (processedNonTextKeys.has(key)) continue;
        const cs = window.getComputedStyle(svg);
        if (cs.display === 'none' || cs.visibility === 'hidden' || cs.opacity === '0') continue;
        // Skip if inside footer or partner section
        if (svg.closest('.logos, .partners, footer ul, footer')) continue;
        // Get fill color from SVG
        let svgFillColor: string | null = null;
        const gFill = svg.querySelector('g[fill]');
        if (gFill) svgFillColor = gFill.getAttribute('fill');
        if (!svgFillColor) {
            const paths = svg.querySelectorAll('path[fill]');
            if (paths.length > 0) svgFillColor = (paths[0] as Element).getAttribute('fill');
        }
        if (!svgFillColor) svgFillColor = svg.getAttribute('fill');
        // Determine bg color at center of SVG
        const bgColor = getVisualBackgroundAtPoint(rect.x + rect.width / 2, rect.y + rect.height / 2, null, true);
        const parentLink = svg.closest('a') as HTMLAnchorElement | null;
        nonTextElements.push({
    ...
    for (const hamburger of document.querySelectorAll('.hamburger, .hamburger-container, [class*="hamburger"], [class*="nav-toggle"], button.menu-toggle') as NodeListOf<HTMLElement>) {
        ...
        if (!lineColor) continue; // Can't determine color, skip
        const bgColor = getVisualBackgroundAtPoint(rect.x + rect.width / 2, rect.y + rect.height / 2, null, true);
        nonTextElements.push({
            type: 'hamburger-icon',
