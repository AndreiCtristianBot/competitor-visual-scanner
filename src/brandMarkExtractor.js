export async function captureSiteBrandMark(page, strategy, runSafe, viewport) {
    let siteLogo = null;
    try {
        ...
        const logoInfo = await page.evaluate(() => {
            ...
            for (const sel of imgSelectors) {
                ...
                if (!el)
                    continue;
                if (el.closest('.logos, .partners, .footer-logos, footer ul'))
                    continue;
                ...
                if (rect.width > 10 && rect.height > 10) {
                    return { type: 'img', src: el.src || '', x: rect.x, y: rect.y, width: rect.width, height: rect.height };
                }
            }
            ...
            for (const sel of svgSelectors) {
                ...
                if (!el)
                    continue;
                if (el.closest('.logos, .partners, footer ul'))
                    continue;
                const elHtml = el.closest('.logo, header');
                if (elHtml) {
                ...
                if (rect.width > 10 && rect.height > 10) {
                    ...
                    if (gFill)
                        svgFill = gFill.getAttribute('fill') || '';
                    if (!svgFill) {
                        const paths = el.querySelectorAll('path[fill]');
                        if (paths.length > 0)
                            svgFill = paths[0].getAttribute('fill') || '';
                    }
                    if (!svgFill)
                        svgFill = el.getAttribute('fill') || '';
                    return { type: 'svg', src, x: rect.x, y: rect.y, width: rect.width, height: rect.height, svgFill };
                }
            }
            ...
            for (const sel of bgSelectors) {
                ...
                if (!el)
                    continue;
                if (el.closest('.logos, .partners, footer'))
                    continue;
                const cs = window.getComputedStyle(el);
                ...
                if (hasBgImg) {
                    ...
                    return { type: 'css-bg', src: bgUrl, x: rect.x, y: rect.y, width: rect.width, height: rect.height, cssFilter };
                }
                // Text-replacement logo (Palace: a.logo with visibility:hidden text "GPA")
                if (rect.width < 400 && rect.height < 300) {
                    return { type: 'text-logo', src: '', x: rect.x, y: rect.y, width: rect.width, height: rect.height };
                }
            }
            return null;
        });
        ...
        if (logoInfo) {
            ...
            if (logoInfo.type === 'svg' && logoInfo.src) {
                ...
                const svgFill = (logoInfo.svgFill || '').toLowerCase().trim();
                ...
                await page.evaluate((svgSrc, w, h, bg) => {
                ...
                const logoBuf = await page.screenshot({ type: 'png', clip });
                await page.evaluate(() => { const el = document.getElementById('__logo_temp'); if (el)
                    el.remove(); });
                siteLogo = {
                ...
                if (siteLogo)
                    console.log(`[Logo] Found SVG: ${siteLogo.width}x${siteLogo.height} fill=${svgFill || 'none'} bg=${logoBgColor}`);
            }
                ...
                const cssFilter = logoInfo.cssFilter || '';
                ...
                await page.evaluate((bgUrl, w, h, filter) => {
                    const old = document.getElementById('__logo_temp');
                    if (old)
                        old.remove();
                    const c = document.createElement('div');
                ...
                const captured = await page.evaluate(() => {
                    ...
                    for (const s of selectors) {
                        ...
                        if (el && !el.querySelector('img, svg') && !el.closest('.logos, footer'))
                            break;
                        el = null;
                    }
                    if (!el)
...
//# sourceMappingURL=brandMarkExtractor.js.map