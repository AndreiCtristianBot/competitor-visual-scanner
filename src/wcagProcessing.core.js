...
export function deduplicateFinalArrows(allTextNodes) {
...
function selectFrame(node, captureData, finalSectionBuffer) {
...
function isLikelyBlankCrop(img) {
    const w = img.width;
    const h = img.height;
    if (w < 6 || h < 6)
        return true;
...
async function cropPreview(buffer, rect, isArrow) {
    try {
        ...
        for (const pad of padCandidates) {
            ...
            if (cropX + cropW > VIEWPORT_W)
                cropW = VIEWPORT_W - cropX;
            if (cropY + cropH > VIEWPORT_H)
                cropH = VIEWPORT_H - cropY;
            if (cropW <= 5 || cropH <= 5)
                continue;
            const crop = frameImage.clone().crop({ x: cropX, y: cropY, w: cropW, h: cropH });
            if (isLikelyBlankCrop(crop))
                continue;
            return await crop.getBase64("image/jpeg");
        }
    }
    catch (err) {
        console.error('Frame crop error:', err);
    }
    return undefined;
}
 ...
 */
async function inferSolidBgFromFrame(buffer, rect) {
    try {
...
export async function processWCAG(textNodes, captureData, finalSectionBuffer, nonTextOverlayElements) {
    const issuesMap = new Map();
    const seenSemanticIssueKeys = new Set();
    const fontGroups = {};
    const textColorAgg = {};
    ...
    if (nonTextOverlayElements) {
        for (const el of nonTextOverlayElements) {
            if (el.type !== 'inline-svg-icon' && el.type !== 'hamburger-icon')
                continue;
    ...
    for (const node of textNodes) {
        if (node.fontFamily?.length > 2) {
            const key = `${node.fontFamily} (${node.fontWeight})`;
            fontGroups[key] = (fontGroups[key] || 0) + 1;
        }
        if (node.textColor) {
            if (!textColorAgg[node.textColor])
                textColorAgg[node.textColor] = { score: 0, elements: new Set() };
        ...
        if (['icon', 'icomoon', 'awesome'].some(bad => node.fontFamily.toLowerCase().includes(bad)))
            continue;
        let statusAA = 'PASS';
        let statusAAA = 'PASS';
        let contrastDisplay = "";
        ...
        if (isImage) {
            ...
            if (inferredSolidBg) {
                try {
                    ...
                    else if (contrast < reqAAA) {
                        statusAAA = 'FAIL';
                        note = "Passes AA, Fails AAA";
                    }
                    else {
                        note = "Solid overlay detected (not image background).";
                    }
        ...
        if (statusAA !== 'PASS' || statusAAA !== 'PASS') {
            ...
            if (seenSemanticIssueKeys.has(semanticIssueKey))
                continue;
...
export async function processNonTextElements(elements, captureData, finalSectionBuffer) {
    const results = [];
    ...
    for (const el of elements) {
                ...
                try {
                    ...
                    if (isBlank && finalImg) {
                        try {
                            ...
                            console.error(`[NonText-Crop] ${el.label}: Final section crop failed:`, err);
                        }
                    }
                    ...
                    console.error('[NonText] Screenshot crop error:', err);
                }
            }
            else {
        ...
        if (fgColor && bgColor && !isImageBg) {
            try {
                ...
                status = 'WARNING';
                note = 'Could not compute contrast';
            }
        }
        else if (isImageBg) {
        ...
        let stickyPreviewBase64;
        let stickyContrastRatio;
        let stickyWcag1411;
        let stickyEnhanced;
        if (el.stickyScreenshot) {
            try {
                stickyPreviewBase64 = await (await Jimp.read(el.stickyScreenshot)).getBase64("image/png");
            }
            catch { }
        }
        if (fgColor && el.stickyBgColor) {
            try {
                ...
                console.log(`[NonText-Sticky-WCAG] ${el.label}: ERROR: ${e?.message}`);
            }
        }
        else {
...
//# sourceMappingURL=wcagProcessing.core.js.map