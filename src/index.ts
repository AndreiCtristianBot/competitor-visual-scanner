...
// 1. Adăugăm keepCookies în definiția de tip
...
app.post(
  ...
  async (req: Request<{}, {}, AnalyzeRequestBody>, res: Response) => {
    try {
      ...
      const finalData = results.map((r: PromiseSettledResult<any>, i: number) => {
        if (r.status === 'fulfilled') return r.value;
        console.error(
          `[Eroare la procesare] URL: ${urls[i]}, Motiv: ${r.reason?.message ?? String(r.reason)}`
        );
        return {
          url: urls[i],
