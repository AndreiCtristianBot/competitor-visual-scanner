...
import { scanViewportChunk } from './viewportChunkScanner.js';
...
export async function runAnalysisCaptureLoop(params) {
    ...
    const captureData = [];
    const globalBgCounts = {};
    const globalImageCounts = {};
    let allTextNodes = [];
    const allNonTextElements = [];
    let totalScore = 0;
    ...
    while (!reachedEnd && loopCount < 25) {
        ...
        const chunk = await page.evaluate(scanViewportChunk, loopCount, strategy);
        if (chunk) {
            ...
            for (const [k, v] of Object.entries(chunk.localBgs))
                globalBgCounts[k] = (globalBgCounts[k] || 0) + v;
            const imgCount = Object.values(chunk.localImgs).reduce((a, b) => a + b, 0);
            if (imgCount > 0)
                globalBgCounts['IMAGE/VIDEO'] = (globalBgCounts['IMAGE/VIDEO'] || 0) + imgCount;
            for (const [k, v] of Object.entries(chunk.localImgs))
                globalImageCounts[k] = (globalImageCounts[k] || 0) + v;
            if (chunk.hasStaticFinalBehindMain)
                hasStaticFinalBehindMain = true;
            allTextNodes = allTextNodes.concat(chunk.viewTextNodes.map((n) => ({ ...n, hasFinalInViewport: chunk.hasFinalSection })));
        ...
        captureData.push({ buffer: buffer, index: loopCount, x: 0 });
...
//# sourceMappingURL=analysisLoopRunner.js.map